...
# Phase 2: Remove this skip block for Pydantic schema validation (P2-SCHEMA-001)
pytestmark = pytest.mark.skip(reason="Phase 2: Pydantic schema validation needed. Trigger: P2-SCHEMA-001")
...
def test_module_ping_request_schema() -> None:
    ...
    # Test that empty module name raises an error
    with pytest.raises(ValidationError):
...
def test_module_create_schema() -> None:
    """Test the ModuleCreate schema."""
    # Create a valid instance
    module = ModuleCreate(name="cc", version="1.0.0")
    ...
    with pytest.raises(ValidationError):
        ModuleCreate(name="", version="1.0.0")
