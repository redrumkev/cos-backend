...
async def mock_service_interruption(
    service_name: str, 
    interruption_type: str = "connection_error",
...
@asynccontextmanager
async def _mock_redis_interruption(
    interruption_type: str, delay: Optional[float]
) -> AsyncGenerator[None, None]:
    ...
    if interruption_type == "connection_error":
        ...
        with patch("redis.asyncio.Redis.ping", side_effect=conn_error):
            with patch("redis.asyncio.Redis.get", side_effect=conn_error):
                with patch("redis.asyncio.Redis.set", side_effect=conn_error):
                    with patch("redis.asyncio.Redis.publish", side_effect=conn_error):
                        # Mock pubsub instance methods
                        with patch("redis.asyncio.client.PubSub.get_message", side_effect=conn_error):
                            with patch("redis.asyncio.client.PubSub.subscribe", side_effect=conn_error):
                                with patch("redis.asyncio.client.PubSub.unsubscribe", side_effect=conn_error):
        ...
        with patch("redis.asyncio.Redis.ping", side_effect=timeout_side_effect):
            with patch("redis.asyncio.Redis.get", side_effect=timeout_side_effect):
                with patch("redis.asyncio.Redis.set", side_effect=timeout_side_effect):
                    with patch("redis.asyncio.Redis.publish", side_effect=timeout_side_effect):
                        mock_pubsub = AsyncMock()
        ...
        with patch("src.common.pubsub.get_pubsub", return_value=mock_pubsub):
            ...
            with patch("redis.asyncio.Redis.pubsub") as mock_redis_pubsub:
...
@asynccontextmanager  
async def _mock_postgres_interruption(
    interruption_type: str, delay: Optional[float]
) -> AsyncGenerator[None, None]:
    ...
    if interruption_type == "connection_error":
        ...
        with patch("sqlalchemy.ext.asyncio.AsyncSession.execute", side_effect=OperationalError("Mocked DB connection failure", None, None)):
            with patch("sqlalchemy.ext.asyncio.AsyncSession.commit", side_effect=DisconnectionError("Database disconnected")):
                with patch("sqlalchemy.ext.asyncio.AsyncSession.rollback", side_effect=DisconnectionError("Database disconnected")):
                    yield
    else:
        # No mocking for unknown types
        yield
