...
@app.get("/ping", response_model=dict[str, str])  # type: ignore[misc]
def ping() -> dict[str, str]:
    return {"status": "mem0 alive"}


@app.get("/memory/{memory_id}", response_model=dict[str, Any])  # type: ignore[misc]
def read_memory(memory_id: str) -> dict[str, Any]:
    ...
    try:
        # Explicitly type the loaded data
        data: dict[str, Any] = json.loads(file_path.read_text(encoding="utf-8"))
        return data
    except json.JSONDecodeError as e:
        raise HTTPException(
            status_code=500, detail="Failed to decode memory JSON"
        ) from e
    except Exception as e:
        raise HTTPException(status_code=500, detail="Failed to read memory file") from e


@app.post("/memory/{memory_id}", response_model=dict[str, Any])  # type: ignore[misc]
def write_memory(memory_id: str, data: dict[str, Any]) -> dict[str, Any]:
    file_path = MEMORY_PATH / f"{memory_id}.json"
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to write memory file"
        ) from e
